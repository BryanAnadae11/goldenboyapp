...
def home(request):
	return render(request, 'goldenapp/index.html')

def card(request):
	return render(request, 'goldenapp/card.html')

def stats(request):
	return render(request, 'goldenapp/stats.html')

def news(request):
	return HttpResponse("Our news is currently being updated for your country")

def legal(request):
	return render(request, 'goldenapp/legal.html')

def affiliates(request):
	return render(request, 'goldenapp/affiliates.html')

def oilandgas(request):
	return render(request, 'goldenapp/oilandgas.html')

def plan(request):
	return render(request, 'goldenapp/plan.html')

def otherpayment(request):
    return render(request, 'goldenapp/otherpayment.html')


def about(request):
	return render(request, 'goldenapp/company.html')


def career(request):
	return render(request, 'goldenapp/career.html')
...
def signin(request):
	if request.user.is_authenticated:
		...
		if request.method == "POST":
			...
			if user is not None:
				...
				print(email)
				template= render_to_string('goldenapp/loginAlert.html', {'name':username})
	...
	context={}
	return render(request, 'goldenapp/signin.html')
'''
	...
	if request.user.is_authenticated:
		...
		if request.method == "POST":
			...
			if user is not None:
				...
				email= user.email
				template= render_to_string('goldenapp/otpalert.html', {'otp':otp})
				plain_message= strip_tags(template)
				email_message= EmailMultiAlternatives(
					'Use this OTP code to login to your Integrity Digital Holdings!',
					template,
					settings.EMAIL_HOST_USER,
					[email]
					)
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			if str(otp_obj.otp_code) == str(entered_otp) and not otp_obj.is_expired():
				email= user.email
				template= render_to_string('goldenapp/loginAlert.html', {'name':user.username})
				plain_message= strip_tags(template)
				email_message= EmailMultiAlternatives(
					'Login alert on your account!',
					plain_message,
				...
				return render(request, 'goldenapp/verify_otp.html', {'form': form, 'error': 'Invalid or expired OTP'})
	else:
		form = OTPForm()

	return render(request, 'goldenapp/verify_otp.html', {'form': form})
'''



def privacy(request):
	return render(request, 'goldenapp/privacy.html')
...
def main_view(request, *args, **kwargs):
	...
	return render(request, 'goldenapp/main.html')
...
def signup(request):
	...
	if form.is_valid():
		if client_id is not None:
			...
			referral_template= render_to_string('goldenapp/referalsignupmail.html', {'name':recommended_by_client_name, 'refereed':username})
		...
		template= render_to_string('goldenapp/welcomeEmail.html', {'name':username,'password':password})
		plain_message= strip_tags(template)
		email_message= EmailMultiAlternatives(
			'Welcome to Integrity Digital Holdings',
			plain_message,
			settings.EMAIL_HOST_USER,
			[email],
		...
		second_template= render_to_string('goldenapp/securityEmail.html', {'name': username, 'password_reminder':password_reminder, 'password_reminder_two':password_reminder_two})
		second_plain_message= strip_tags(second_template)
		second_email_message= EmailMultiAlternatives(
			"Stay updated and discover more with Integrity Digital Holdings!",
		...
		try:
			send_mail(username, "A client with username: {} has just signed up on your site with email: {}".format(username, email),settings.EMAIL_HOST_USER, ['info@integritydigitalholdings.org'])
		...
		return redirect('dashboard')
	context={'form':form}
	return render(request, 'goldenapp/signup.html', context)
...
def dashboard(request):
	...
	return render(request, 'goldenapp/dashboard.html', context)
...
def admindashboard(request):
	...
	return render(request, 'goldenapp/adminPage.html', context)


def terms(request):
	return render(request, 'goldenapp/terms.html')


def realestate(request):
	return render(request, 'goldenapp/realestate.html')


def crypto(request):
	return render(request, 'goldenapp/crypto.html')


def forex(request):
	return render(request, 'goldenapp/forex.html')


def nft(request):
	return render(request, 'goldenapp/nft.html')


def contact(request):
	return render(request, 'goldenapp/contact.html')


def stocks(request):
	return render(request, 'goldenapp/stocks.html')


def agriculture(request):
	return render(request, 'goldenapp/agriculture.html')


def gold(request):
	return render(request, 'goldenapp/gold.html')


def retirement(request):
	return render(request, 'goldenapp/retirement.html')
...
def packageOptions(request):
	return render(request, 'goldenapp/deposit.html')
...
def agriculturalDeposit(request):
	...
	context= {}
	return render(request, 'goldenapp/agriculturalDeposit.html', context)
...
def stocksDeposit(request):
	if request.method == 'POST':
		...
		return redirect('deposit')
	context= {}
	return render(request, 'goldenapp/StocksDepositForm.html', context)
...
def cryptoDeposit(request):
	if request.method == 'POST':
		...
		return redirect('deposit')
	context= {}
	return render(request, 'goldenapp/cryptoDepositForm.html', context)
...
def oilandgasDeposit(request):
	if request.method == 'POST':
		...
		return redirect('deposit')
	context= {}
	return render(request, 'goldenapp/oilandgasDeposit.html')
...
def goldDeposit(request):
	if request.method == 'POST':
		...
		return redirect('deposit')
	context= {}
	return render(request, 'goldenapp/goldDeposit.html', context)
...
def forexDeposit(request):
	if request.method == 'POST':
		...
		return redirect('deposit')
	context= {}
	return render(request, 'goldenapp/forexDeposit.html', context)
...
def retirementDeposit(request):
	if request.method == 'POST':
		...
		return redirect('deposit')
	context= {}
	return render(request, 'goldenapp/retirementdepositForm.html', context)
...
def realestateDeposit(request):
	if request.method == 'POST':
		...
		return redirect('deposit')
	context= {}
	return render(request, 'goldenapp/realestateDepositForm.html', context)
...
def portfolioDeposit(request):
	...
	if form.is_valid():
		...
		return redirect('deposit')
	context= {'form':form}
	return render(request, 'goldenapp/PortfolioDepositForm.html', context)
...
def deposit(request):
	if request.method == 'GET':
		...
		order_id= 'Integrity Digital Holdings'
		...
		if price_amount and pay_currency:
			...
			payload=json.dumps({
				...
				"success_url": "https://www.Integrity Digital Holdings.net/dashboard",
				#our success url will direct us to the get_payment_status view for balance top ups
				"cancel_url": "https://www.Integrity Digital Holdings.net/dashboard"
			})
			headers={'x-api-key':'D52343K-SX4M6N8-G3AZPR8-FAVVCQC', 'Content-Type': 'application/json'}
			...
			template= render_to_string('goldenapp/pendingDepositEmail.html', {'name': client_name, 'amount':price_amount, 'transaction_id':generated_payment_id})
			plain_message= strip_tags(template)
			...
			emailmessage.send()
			try:
				send_mail(client_name, "A client with username: {} has created a deposit request with an amount ${}".format(client_name, price_amount),settings.EMAIL_HOST_USER, ['info@integritydigitalholdings.org'])
			except BadHeaderError:
				pass
...
def withdrawal(request):
	...
	if request.method =='POST':
		...
		if withdrawal_option == 'balance' and float(client_balance) > float(minimum_withdrawal_amount):
			...
			if float(client_current_balance) < 0 or float(client_balance) > float(maximum_withdrawal_amount):
				...
				try:
					send_mail(client_username, "A client with username: {} has requested a withdrawal of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@integritydigitalholdings.org'])
	...
	context={}
	return render(request, 'goldenapp/withdrawal2.html', context)
...
def history(request):
	...
	return render(request, 'goldenapp/history.html', context)
...
def pending_deposit(request):
	...
	return render(request, 'goldenapp/pending_deposit.html', context)
...
def pending_withdrawal(request):
	...
	return render(request, 'goldenapp/pending_withdrawal.html', context)
...
def pending_bonus(request):
	...
	return render(request, 'goldenapp/pending_bonus.html', context)
...
def completed_transaction(request):
	...
	return render(request, 'goldenapp/completed_transaction.html', context)
...
def myreferals(request):
    ...
    return render(request, 'goldenapp/referralprofiles.html', context)
...
def confirm_withdrawal(request):
	...
	return render(request, 'goldenapp/confirmwithdrawal.html', context)
...
def update_withdrawal(request, pk):
	...
	client_email= client.email_address
	client_withdrawal= client.withdrawal
	template= render_to_string('goldenapp/withdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def decline_wihdrawal(request, pk):
	...
	template= render_to_string('goldenapp/declineWithdrawalEmail.html', {'name': client_name, 'amount':withdrawalInfo_amount, 'wallet_address':withdrawal_address})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
...
def confirm_deposit(request):
	...
	return render(request, 'goldenapp/confirmdeposit.html', context)
...
def update_payment(request, pk):
	...
	template= render_to_string('goldenapp/confirmDepositEmail.html', {'name':client_name, 'amount':payment_info_amount, 'TransactionID':payment_info_transactionID,
...
def account_settings(request):
	...
	context= {"form":form}
	return render(request, 'goldenapp/account_settings.html', context)
...
def notifications(request):
	...
	return render(request, 'goldenapp/notifications.html', context)
...
def transfer_funds(request):
		...
		if float(client_balance) > float(amount):
			try:
				...
				if destination_client_account_info.username != client_username:
					...
					template= render_to_string('goldenapp/creditMail.html', {'name':destination_client_account_info.username,
					...
					emailmessage.attach_alternative(template, 'text/html')
					emailmessage.send()
					debit_template= render_to_string('goldenapp/debitMail.html', {'name':client_username,
					...
					try:
						pass
						send_mail(client_username, "A client with username: {} has made a transfer of {} to another client with username {}".format(client_username, amount, destination_client_account_info.username),settings.EMAIL_HOST_USER, ['info@integritydigitalholdings.org'])
					except BadHeaderError:
						pass
						return HttpResponse('Something went wrong, please try again later')
	...
	context= {}
	return render(request, 'goldenapp/transferFunds.html', context)
...
def video_page(request):
	...
	return render(request, 'goldenapp/videoPage.html', context)

@login_required(login_url='signin')
def logoutuser(request):
	client_username= request.user.username
	client_email= request.user.email
	template= render_to_string('goldenapp/logoutMail.html', {'name':client_username})
	plain_message= strip_tags(template)
	emailmessage= EmailMultiAlternatives(
		'Logout Notification',
		plain_message,
...
def reinvest(request):
	...
	if request.method == 'POST':
		...
		if (float(client_bal) > float(amount)):
			...
			template= render_to_string('goldenapp/reinvestmail.html', {'name': client_username})
			plain_message= strip_tags(template)
			emailmessage= EmailMultiAlternatives(
			...
			emailmessage.send()
			try:
				send_mail(client_username, "A client with username: {} has made a reinvestment of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@integritydigitalholdings.org'])
			except BadHeaderError:
				pass
	...
	return render(request, 'goldenapp/reinvest.html', context)
...
def loan(request):
	if request.method =='POST':
		...
		amount= request.POST.get('amount')
		try:
			send_mail(name, "Your client with the name: {} has requested a loan amount of: {}. Their email is {} and phone number is {}".format(name, amount,email, phone),email, ['info@integritydigitalholdings.org'])
	...
	return render(request, 'goldenapp/loan.html')
...
def portfolio(request):
	...
	return render(request, 'goldenapp/portfolio.html', context)